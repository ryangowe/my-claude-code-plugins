...
def test_finds_edited_py_files(make_transcript):
    ...
    assert result["py_files"] == ["/src/main.py"]


def test_ignores_non_py_files(make_transcript):
    entries = [_user("fix it"), _assistant(_edit_block("/src/config.toml"))]
    result = require_review.check(make_transcript(entries))
    assert result["py_files"] == []


def test_finds_written_py_files(make_transcript):
    entries = [_user("create it"), _assistant(_write_block("/src/new.py"))]
    result = require_review.check(make_transcript(entries))
    assert result["py_files"] == ["/src/new.py"]
...
def test_deduplicates_files(make_transcript):
    ...
    result = require_review.check(make_transcript(entries))
    assert result["py_files"] == ["/a.py"]


# -- _reviewer_spawned ---------------------------------------------------------


def test_detects_reviewer_agent(make_transcript):
    entries = [
        _user("fix"),
        _assistant(_edit_block("/a.py")),
        _assistant(_agent_block("python-engineering:python-change-reviewer")),
    ]
    result = require_review.check(make_transcript(entries))
...
def test_ignores_other_agents(make_transcript):
    entries = [
        _user("fix"),
        _assistant(_edit_block("/a.py")),
...
def test_tool_result_does_not_break_turn(make_transcript):
    ...
    result = require_review.check(make_transcript(entries))
    assert sorted(result["py_files"]) == ["/a.py", "/b.py"]


# -- _block_decision -----------------------------------------------------------


def test_blocks_when_py_edited_without_review():
    result = {"py_files": ["/a.py"], "reviewed": False}
...
def test_no_block_when_reviewed():
    result = {"py_files": ["/a.py"], "reviewed": True}
    assert require_review._block_decision(result) is None


def test_no_block_when_no_py_files():
    result = {"py_files": [], "reviewed": False}
    assert require_review._block_decision(result) is None
...
def test_main_blocks_on_unreviewed_py_edit(make_transcript, monkeypatch, capsys):
    entries = [_user("fix"), _assistant(_edit_block("/a.py"))]
