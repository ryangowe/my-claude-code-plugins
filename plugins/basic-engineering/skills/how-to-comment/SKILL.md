...
两个原则：快照化（snapshotting）与注意力（attention，即少写注释）。原则与语言无关，示例以 Python 举例。
...
任何要提交进 git 的内容都应当是某个状态的快照，而不是两个 commit 之间的差异。对注释来说，合法的内容只有三类：
...
### Inline comments: only write what the code can't show
...
### Docstrings: keep short by default

- 优先将契约代码化而非写 docstrings
- 更多依靠类型签名与命名表达语义
- 不要用 docstring 重新描述代码做了什么
- docstrings 更新频率应当低于代码，它们对具体实现的依赖应该低于代码接口
...
当 docstring 的内容已经超出「代码接口的补充说明」——比如需要写使用示例、解释设计决策、描述整体架构——它实际上是一份内嵌的文档。这时 Load how-to-document，用文档的原则来写。

# Examples

以下示例用 Python，但规则适用于任何语言。

## Bad: commit diff leaked into snapshot
...
## Good: trade-off relative to default approach
...
## Bad: restating implementation steps
...
## Good: one line that says what it does
...
## Good: only the WHY the signature can't show
...
## Bad: hardcoded values that drift
