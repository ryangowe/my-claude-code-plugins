<!--
模板:RFC / 方案稿(供 review,通常先于实现)。对应 spec/ 里的 agent-integration。
来源:Rust RFC 模板(github.com/rust-lang/rfcs/blob/master/0000-template.md)。
以下英文正文逐字保留,不预先翻译——翻译错误一旦写进模板会被反复复用,代价大于临用翻译。

使用时(由 create-spec 执行):
  1. 落盘到 spec/<今天日期>-<主题>.md
  2. 把下面英文正文翻译成中文,并套用 lotra 约定:
     - Unresolved questions → `## 待定`
     - 本次不做的 → `## 不在本次范围`
     - 顶部 Rust 专属头部(RFC PR / Rust Issue)删掉,Start Date 用今天
  3. 删掉本注释
-->
...
Any changes to Rust should focus on solving a problem that users of Rust are having. This section should explain this problem in detail, including necessary background.
